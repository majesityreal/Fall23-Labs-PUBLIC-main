...
d3.csv('cars.csv', dataPreprocessor).then(function(dataset) {
        ...
        chartG.selectAll('.y.axis')
            ...
            d3.select('#x-axis-select').on('change', function() {
                scatterXAttr = this.value;
                scatterCell.prototype.update;
                BarChartCell.prototype.update;
            });

            d3.select('#y-axis-select').on('change', function() {
                scatterYAttr = this.value;
                scatterCell.prototype.update;
                BarChartCell.prototype.update;
            });
...
function brushmove(event, cell) {
    ...
    if(e) {
        ...
        if (event.sourceEvent.screenX > 365) {
            svg.selectAll(".bar")
            .classed("hidden", function(d){
                var barY = cellHeight - d[1] - 10;
                var barX = 50 * d[0] - 150;
                // if the first x is lower than the first
                var boolean = e[0][0] > barX || barX > e[1][0]
                || e[0][1] > barY || barY > e[1][1];
                // if not hidden
                if (!boolean) {
                    svg.selectAll(".dot")
                    .classed("hidden", function(f){
                        if (f['cylinders'] == d[0]) {
                            return false;
                        }
                        // if the first x is lower than the first
        ...
        // left side
            ...
            svg.selectAll(".dot")
            .classed("hidden", function(d){
                var boolean = e[0][0] > xScale(d[cell.x]) || xScale(d[cell.x]) > e[1][0]
